...
describe("SALE_CHAIN client selector", () => {
  ...
  it("defaults to mainnet when unset (documented prod default)", async () => {
  ...
  // The failure this guards: any typo/case slip used to fall through `?? mainnet` SILENTLY - the
  // client would frame mainnet while the server (z.enum, fail-closed) audits sepolia. Unknown
  // set values must fail the build, never pick a chain.
  it("throws at import on an unknown value instead of failing open to mainnet", async () => {
    vi.stubEnv("NEXT_PUBLIC_SALE_CHAIN", "Sepolia")
