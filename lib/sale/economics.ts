...
export const SALE_ECONOMICS = {
  ...
  // gate money. LOUD WARNING: keep in sync with the Sonar dashboard (no SDK endpoint
  // exposes them) - if Sonar shifts the schedule, ALL page copy/countdowns/phases here drift. The
  // push/email crons no longer trust these dates (chain-first via lib/sale/live-window.ts); the
  // UI phase still does.
