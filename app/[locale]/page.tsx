import { type FaqTranslator, buildFaq, faqAnswerText } from "@/content/sections/faq"
...
export default async function Home({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // FAQPage structured data, sourced from the same buildFaq() the rendered FAQ uses so it stays in
  // sync and localized. Answers are flattened to plain text (Google requires plain text/limited
  // HTML in acceptedAnswer.text). Rendered as a <script> text child, matching the
  // Organization/WebSite block in the layout (never dangerouslySetInnerHTML).
  const tFaq = await getTranslations({ locale, namespace: "Faq" })
  const faq = buildFaq(tFaq as unknown as FaqTranslator, locale)
  const faqJsonLd = {
