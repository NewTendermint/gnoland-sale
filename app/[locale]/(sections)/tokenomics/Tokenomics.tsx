import {
  TOTAL_SUPPLY,
  allocation,
  circulating,
  circulatingBreakdown,
} from "@/content/sections/tokenomics"
// Tokenomics: Allocation + Vesting breakdown tiles (no visible heading, sr-only h2).
import { useTranslations } from "next-intl"
import { ClipOpen } from "../../(ui)/ClipOpen"
import { DrawLine } from "../../(ui)/DrawLine"
import { FadeIn } from "../../(ui)/FadeIn"
import { RevealGroup } from "../../(ui)/RevealGroup"
import { Section } from "../../(ui)/Section"

const gnot = (n: number) => `${n.toLocaleString("en-US")} GNOT`

export function Tokenomics() {
  const t = useTranslations("Tokenomics")
  const circulatingPct = ((circulating.total / TOTAL_SUPPLY) * 100).toFixed(1)
  const lockedPct = (100 - Number(circulatingPct)).toFixed(1)

  // Waffle: 1 cell = 1% of total supply.
  const tokenSaleCells = Math.round((circulating.tokenSaleSupply / TOTAL_SUPPLY) * 100)
  const circulatingCells = Math.round((circulating.total / TOTAL_SUPPLY) * 100)
  const tokenSalePct = ((circulating.tokenSaleSupply / TOTAL_SUPPLY) * 100).toFixed(1)
  const otherPct = (
    ((circulating.total - circulating.tokenSaleSupply) / TOTAL_SUPPLY) *
    100
  ).toFixed(1)
  const waffleCells = Array.from({ length: 100 }, (_, i) => {
    if (i < tokenSaleCells) {
      return { title: t("cellTokenSale", { pct: tokenSalePct }), color: "var(--foreground)" }
    }
    if (i < circulatingCells) {
      return {
        title: t("cellOtherCirculating", { pct: otherPct }),
        color: "color-mix(in srgb, var(--foreground) 40%, transparent)",
      }
    }
    return {
      title: t("cellLocked", { pct: lockedPct }),
      color: "color-mix(in srgb, var(--foreground) 8%, transparent)",
    }
  })

  const circFacts: Array<{ id: string; label: string; value: string; light?: boolean }> = [
    { id: "lockup", label: t("lockupLabel"), value: t("lockupValue"), light: true },
    {
      id: "fdv",
      label: t("fdvLabel"),
      value: `$${circulating.fdvUsd.toLocaleString("en-US")}`,
    },
  ]

  return (
    <Section id="tokenomics" gridClassName="-mt-6 lg:-mt-14">
      <RevealGroup inline staggerMs={70} fromBottomPct={40}>
        <h2 className="sr-only">{t("srHeading")}</h2>

        <RevealGroup as="div" className="col-span-12 flex flex-col lg:col-span-5 lg:col-start-2">
          <ClipOpen
            index={1}
            className="flex flex-1 flex-col rounded-[var(--frame-radius)] bg-surface-alt p-6 text-foreground sm:p-8 lg:p-10"
          >
            <FadeIn as="div" index={8} className="mb-4 flex items-baseline justify-between gap-3">
              <p className="font-mono text-sm font-bold uppercase tracking-widest text-foreground">
                {t("distributionTitle")}
              </p>
              <p className="font-mono text-xs uppercase tracking-widest text-muted tabular-nums">
                {gnot(TOTAL_SUPPLY)}
              </p>
            </FadeIn>

            <ul className="mt-6 flex flex-1 flex-col justify-between">
              {allocation.map((row, i) => (
                <FadeIn as="li" key={row.id} index={9 + i} className="py-2">
                  <div className="flex items-baseline justify-between gap-3 text-sm">
                    <span className="font-mono uppercase tracking-widest text-foreground">
                      {t(`alloc.${row.id}.category`)}
                    </span>
                    <span className="font-mono font-bold tabular-nums uppercase tracking-widest text-foreground">
                      {row.percent.toFixed(2)}%
                    </span>
                  </div>
                  <div
                    role="img"
                    aria-label={t("allocationAria", {
                      category: t(`alloc.${row.id}.category`),
                      percent: row.percent,
                    })}
                    className="mt-2 h-2 w-full overflow-hidden rounded-sm bg-[color-mix(in_srgb,var(--foreground)_8%,transparent)]"
                  >
                    <div
                      className="h-full rounded-sm bg-foreground"
                      style={{ width: `${row.percent}%` }}
                    />
                  </div>
                  <div className="mt-1.5 flex items-baseline justify-between gap-3">
                    <span className="text-xs text-muted">{t(`alloc.${row.id}.note`)}</span>
                    <span className="shrink-0 font-mono text-xs tabular-nums text-muted">
                      {row.amount.toLocaleString("en-US")}
                      {row.hasFootnote ? <span aria-hidden="true">*</span> : null}
                    </span>
                  </div>
                </FadeIn>
              ))}
            </ul>

            {allocation.some((row) => row.hasFootnote) ? (
              <FadeIn as="div" index={16} className="mt-4 flex flex-col gap-y-1">
                {allocation
                  .filter((row) => row.hasFootnote)
                  .map((row) => (
                    <p key={row.id} className="text-xs text-muted">
                      *{t(`alloc.${row.id}.footnote`)}
                    </p>
                  ))}
              </FadeIn>
            ) : null}
          </ClipOpen>
        </RevealGroup>

        <RevealGroup as="div" className="col-span-12 flex flex-col lg:col-span-5 lg:col-start-7">
          <ClipOpen
            index={1}
            className="flex flex-1 flex-col rounded-[var(--frame-radius)] bg-surface-alt p-6 text-foreground sm:p-8 lg:p-10"
          >
            <FadeIn as="div" index={8} className="mb-4 flex items-baseline justify-between gap-3">
              <p className="font-mono text-sm font-bold uppercase tracking-widest text-foreground">
                {t("circulatingTitle")}
              </p>
              <p className="font-mono text-xs uppercase tracking-widest text-muted tabular-nums">
                {t("circulatingAtTge", { pct: circulatingPct })}
              </p>
            </FadeIn>

            <FadeIn
              as="div"
              index={9}
              role="img"
              aria-label={t("waffleAria", {
                total: TOTAL_SUPPLY.toLocaleString("en-US"),
                circulating: circulatingPct,
                locked: lockedPct,
              })}
              className="grid w-full grid-cols-[repeat(20,minmax(0,1fr))] gap-1"
            >
              {waffleCells.map((cell, i) => (
                <div
                  // biome-ignore lint/suspicious/noArrayIndexKey: cells are positional by design
                  key={i}
                  title={cell.title}
                  className="aspect-square rounded-[2px]"
                  style={{ backgroundColor: cell.color }}
                />
              ))}
            </FadeIn>

            <FadeIn
              as="div"
              index={9}
              className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[10px] uppercase tracking-[0.15em] text-muted"
            >
              <span className="flex items-center gap-1.5">
                <span
                  aria-hidden="true"
                  className="size-2 shrink-0 rounded-[1px]"
                  style={{ backgroundColor: "var(--foreground)" }}
                />
                {t("legendTokenSale")}
              </span>
              <span className="flex items-center gap-1.5">
                <span
                  aria-hidden="true"
                  className="size-2 shrink-0 rounded-[1px]"
                  style={{
                    backgroundColor: "color-mix(in srgb, var(--foreground) 40%, transparent)",
                  }}
                />
                {t("legendOtherCirculating")}
              </span>
              <span className="ml-auto flex items-center gap-1.5">
                <span
                  aria-hidden="true"
                  className="size-2 shrink-0 rounded-[1px]"
                  style={{
                    backgroundColor: "color-mix(in srgb, var(--foreground) 14%, transparent)",
                  }}
                />
                {t("legendLocked", { pct: lockedPct })}
              </span>
            </FadeIn>

            <div className="mt-auto flex flex-col pt-7">
              <FadeIn
                as="div"
                index={10}
                className="flex items-baseline justify-between gap-3 pb-2.5 text-sm"
              >
                <span className="font-mono font-bold uppercase tracking-widest text-foreground">
                  {t("totalCirculatingAtTge")}
                </span>
                <span className="font-mono font-bold tabular-nums uppercase tracking-widest text-foreground">
                  {circulating.total.toLocaleString("en-US")}
                </span>
              </FadeIn>

              <DrawLine colorClass="bg-border" index={10} />

              <ul className="mt-2 flex flex-col gap-y-1">
                {circulatingBreakdown.map((row, i) => (
                  <FadeIn as="li" key={row.id} index={11 + i} className="py-1">
                    <div className="flex items-baseline justify-between gap-3 text-sm">
                      <span className="uppercase tracking-wide text-foreground">
                        {t(`circ.${row.id}.category`)}
                      </span>
                      <span className="font-mono font-bold tabular-nums text-foreground">
                        {row.amount.toLocaleString("en-US")}
                      </span>
                    </div>
                    {row.children ? (
                      <ul className="mt-2 ml-px flex flex-col gap-y-1.5 border-l border-border pl-4">
                        {row.children.map((c) => (
                          <li
                            key={c.id}
                            className="flex items-baseline justify-between gap-3 text-xs"
                          >
                            <span
                              className={`uppercase tracking-wide ${c.highlight ? "font-medium text-foreground" : "text-muted"}`}
                            >
                              {t(`circ.${c.id}.category`)}
                            </span>
                            <span
                              className={`font-mono tabular-nums ${c.highlight ? "font-bold text-foreground" : "text-muted"}`}
                            >
                              {c.amount.toLocaleString("en-US")}
                            </span>
                          </li>
                        ))}
                      </ul>
                    ) : null}
                  </FadeIn>
                ))}
              </ul>
            </div>

            <DrawLine className="mt-5" colorClass="bg-border" index={24} />

            <ul className="mt-3 flex flex-col gap-y-2.5">
              {circFacts.map((fact, i) => (
                <FadeIn
                  as="li"
                  key={fact.id}
                  index={25 + i}
                  className="flex items-baseline justify-between gap-4 text-sm"
                >
                  <span className="shrink-0 font-mono uppercase tracking-widest text-muted">
                    {fact.label}
                  </span>
                  <span
                    className={
                      fact.light
                        ? "text-right text-muted"
                        : "text-right font-mono font-bold tabular-nums text-foreground"
                    }
                  >
                    {fact.value}
                  </span>
                </FadeIn>
              ))}
            </ul>
          </ClipOpen>
        </RevealGroup>
      </RevealGroup>
    </Section>
  )
}
