      ...
      <RevealGroup inline>
          ...
          {preSale ? (
            <FadeIn as="div" className="mt-6">
              <p className="text-[10px] uppercase tracking-[0.2em] text-muted">
                {registrationOpen ? t("saleOpensIn") : t("registrationOpensIn")}
              </p>
              <p className="mt-1 font-mono text-4xl font-medium tracking-tight tabular-nums text-foreground sm:text-5xl">
                <Countdown
                  ...
                  label={registrationOpen ? t("saleOpensIn") : t("registrationOpensIn")}
                />
              </p>
            </FadeIn>
          ) : (
                  ...
                  <Fragment key={m.id}>
                    ...
                    <div className="flex flex-col-reverse">
                      ...
                      >
                        ...
                        {m.badge && m.value !== "-" ? t(`status.${m.value}`) : m.value}
                      </dd>
                    </div>
                  </Fragment>
                ))}
        ...
        <RevealBoundary>
          <div className="col-span-12 flex flex-col lg:col-span-10 lg:col-start-2">
            {termGroups.map((g, gi) => (
              <RevealGroup inline fromBottomPct={TABLE_REVEAL_PCT} key={g.id}>
          ...
          <RevealGroup inline fromBottomPct={TABLE_REVEAL_PCT}>
            <div className="col-span-12 mt-6 flex flex-wrap items-center justify-end gap-3 lg:col-span-10 lg:col-start-2">
              {documents.map((d, di) => (
                <FadeIn as="div" index={di} key={d.id}>
                  <Cta
                    href={d.href}
                    external={!d.href.startsWith("#") && !d.download}
                    download={d.download}
                    arrow="diagonal"
                    label={t(`documents.${d.id}.value`)}
                    variant="ghost"
                    size="sm"
                  />
                </FadeIn>
