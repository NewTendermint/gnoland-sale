...
> **2026-06-23 amendment (owner-validated): cap label.** The "Hard cap" terms row is renamed **"Raise target"**; the value stays the exact **$10,000,000** - a single figure, NOT a range (the relayed "5,000,000 to 10,000,000 USDC" from the Dongwon meeting is superseded; a raise target must show an exact price). Rendered copy lives in `content/sections/token-details.ts` (Pricing and Caps group). The `maxPriceUsd` price ceiling ($0.129, also called "hardcap" in the bid UI) is a different concept and is unchanged.

> **2026-06-24 amendment (owner-validated, SUPERSEDES the 2026-06-23 cap-label note above): Raise target is a range $5,000,000 - $10,000,000.** Reversing the prior "single exact figure" call: in a fixed-supply (77,500,000 GNOT) uniform-price auction the total raise = clearing price x supply, and the clearing price is discovered within the [$0.0645 floor, $0.129 ceiling] band, so the raise is inherently a range - about $5.0M if it clears at the floor (full subscription), about $10.0M at the ceiling (= `hardCapUsd`). The relayed Dongwon $5M-$10M is the mechanically correct framing; the single $10M described only the ceiling outcome. Rendered copy in `content/sections/token-details.ts` (Pricing and Caps). The `maxPriceUsd` price ceiling and the per-bidder allocation calc (commitment / clearing) are unchanged - no calc edit needed.

> **2026-06-30 amendment (owner-validated display changes; items needing team/Sonar confirmation are flagged PENDING inline): repricing + Circulating Supply tile.** Rendered copy lives in `content/sections/token-details.ts` (Sale Overview + Pricing and Caps) and the Tokenomics tile (`content/sections/tokenomics.ts` + `app/(sections)/tokenomics/Tokenomics.tsx`). Superseding the rows and amendments above:
>
> - **Sale allocation: 77,500,000 (~5.8%) -> 38,760,000 GNOT (~2.9% of supply)** (halved). `saleSupplyGnot` updated.
> - **Soft cap reintroduced at $2,500,000.** SUPERSEDES "Soft cap removed" (2026-06-21) and "Raise target $5,000,000 - $10,000,000" (2026-06-24). The terms row "Raise target" is renamed **"Soft cap"**, value **$2,500,000** (= $0.0645 floor x 38,760,000 allocation = full subscription at the floor). `softCapUsd` added.
> - **"Max price" CONFIRMED removed (team 2026-06-30): no upper price cap anymore.** Done in the DISPLAY (terms table + FAQ rewritten, no more $0.129). QUEUED for the bid-engine rework (bundled with the increment): drop `maxPriceUsd` from the `BidFlow` band + `calc.snapBidPrice` / `validateBidPrice`, rewrite the bid-panel hint copy that still says "up to the $X maximum", and remove the now-stale `hardCapUsd`.
> - **Bid increment SET to $0.01935 in `bidIncrementUsd` (owner "fait le en dur" 2026-06-30); FAQ + terms derive from it.** Sonar exposes NO increment (verified in `@echoxyz/sonar-core`: no field, no method, no sale-config fetch), so a single configured constant is the simple / secure / perf choice - the authoritative guards are the live Sonar permit (MinPrice/MaxPrice) + the on-chain contract, not this display value. The FAQ worked example confirms a FLOOR-ANCHORED grid (bid $0.1806 = $0.0645 + 6 x $0.01935; clearing $0.1419 = $0.0645 + 4 x $0.01935), which our bid code does NOT implement: `priceUsdToTickUnits` / `validateBidPrice` are anchored-at-0 (`lib/sale/calc.ts` l.85-103), so $0.01935 is off-grid with the $0.0645 floor (0.0645 / 0.01935 = 3.333). PRE-LAUNCH BLOCKER: rework the grid model to floor-anchored AND verify the on-chain tick representation against the deployed SettlementSale (`scripts/probe-sale.mjs`) . The bid-GRID code in `calc.ts` (`validateBidPrice` / `snapBidPrice` / `priceUsdToTickUnits`) is still anchored-at-0 and must move to floor-anchored, with the on-chain encoding verified, before the bid path is correct (gated / pre-launch). Team 2026-06-30 confirms 0.01935 is valid (the floor need NOT be a multiple of the increment); the open piece is the contract's exact on-chain price encoding (step index from floor? micro-USD?) needed to wire `priceUsdToTickUnits`.
> - **Starting price stays $0.0645, no footnote.** The `*` in the brief was a marker on "Remove Max Price" (an action item), not a footnote call on the starting price.
> - **Tokenomics "Vesting" tile -> "Circulating Supply".** The monthly unlock-timeline bars are replaced by a stacked **circulating-at-TGE** bar (same visual language as the Allocation tile) with the **Token Sale** slice highlighted (the participant's own bucket). New facts: Token Sale supply 38,760,000; Lockup "1-Year Lockup for US Investors Only"; Total circulating at TGE; FDV $85,948,500 (= $0.0645 x 1,333,000,000). The unlock-schedule prose is unchanged in FAQ (#11) and module #4b. Total circulating = **197,320,000** (team-confirmed 2026-06-30; the earlier 191,320,000 missed the 4% unlock of the 150M investor pool = 6M, folded into the Investors line). Breakdown: Investors 117,240,000 + Token Sale 38,760,000 (= Investors Pool 156,000,000), Airdrop1-Cosmos 14,000,000, NEWTENDERMINT 13,280,000, Airdrop2-AtomOne 9,240,000, Ecosystem 2,400,000, Core 1,600,000, Validator 800,000.
> - **Public vesting REMOVED.** The public sale has NO lockup: 100% transferable at distribution (TGE). Only US accredited investors carry a flat 12-month lockup (no transfer/sale during it). This SUPERSEDES the module #4b unlock schedule (7% TGE / 7% monthly / 9% final, fully vested 13 months) and the old "1-year lockup, 91% month 13 / 9% month 14" framing FOR THE PUBLIC SALE. The Tokenomics `vesting` / `monthlyUnlocks` exports (`content/sections/tokenomics.ts`) are now obsolete and unused.
> - **FAQ rewritten** (`content/sections/faq.ts`, owner's final wording): "How does the auction work?" (no max-price cap; bids in $0.01935 increments at/above clearing; price steps up when demand fills the 38,760,000 at a step); refund example (bid $0.1806 / clearing $0.1419); "Who can participate?" (US accredited; lockup detail moved to the tokens question); "How much can I commit?" (100 USDC min, no max, at/above $0.0645 in $0.01935 steps); "Which wallets?" (reworded); "When do I receive my tokens?" (no public lockup / US 12-month).
> - **Stale worked examples to fix:** the #3 notes above (Total Committed / Percent Filled, ~line 83-84) still use 77.5M and the $0.129 ceiling (now removed); with 38,760,000 a full subscription at the $0.0645 floor is ~$2.5M (= the soft cap).
...
> **2026-06-23 amendment (owner-relayed, team request): FAQ "Who can participate?" split into two paragraphs.** A paragraph break now sits before the "US participants must be accredited investors" sentence so the US-investor + lockup terms stand out. Wording unchanged; mirrored as a two-element array in `content/sections/faq.ts`.
...
7. **Who can participate?** - Eligibility is determined by your jurisdiction and verified through Sonar during registration. The sale is unavailable in certain regions - if yours is restricted, Sonar will let you know.
...
9. **Which wallets can I use?** (NEW) - You can take part with any self-custody Ethereum wallet that holds USDC and can sign on Ethereum mainnet. The wallets we recommend are MetaMask, Coinbase Wallet, and Rabby, plus any wallet you connect through WalletConnect, such as Rainbow, Trust, or Ledger. When you go to connect, you'll only see the wallets you already have installed in your browser, so set one up before the sale. New to wallets? You can compare and choose one at ethereum.org. (picker mirrors this recommended list; the WalletConnect clause is accurate only once its projectId is set)
10. **How much can I commit?** - The minimum commitment requirement is $100 USDC, and there is no maximum commitment limit. You can bid anywhere between the starting price of $0.0645 and the maximum price of $0.129 in $0.00645 increments. (numbers dynamic from economics; no maximum confirmed 2026-06-21)
11. **When do I receive my tokens?** - Token distribution is set to happen in September. After you receive your tokens, tokens will be transferable with an unlock schedule applied. The unlock schedule is as follows: 7% unlocks at token generation, then 7% each month, with the final 9% unlocked in month 14. There is no cliff. (month dynamic from economics; matches #4b vesting)
12. **Can I withdraw my bid?** - No. You cannot lower or cancel a bid while the sale is running. If your bid ends below the final clearing price, your committed funds are refunded after settlement.
...
**Legal documents** (verbatim, externally authored by the team). To avoid drift, the canonical text lives in `content/legal/*.ts` (NOT duplicated here), rendered via `LegalMarkdown`, linked from the footer (`#16`) - do NOT reword, retitle, or restructure:

- **Privacy Policy** - `content/legal/privacy-policy.ts`, page `/privacy-policy`
- **Terms of Service** (risk disclosures = section 10, anchor `#risk-disclosures`) - `content/legal/terms-of-service.ts`, page `/terms-of-service`
- **U.S. Investor Disclaimer** (updated 2026-06-30; U.S. accredited-investor part + a Non-US Regulation S section) - `content/legal/us-investor-disclaimer.ts`, page `/us-investor-disclaimer`
